...
def mainScreen():
	...
	while action:
		clear()
		curDir = os.getcwd()
		result=pyfiglet.figlet_format("Lol_Sec07")
		print(result)
		
		print(colored('='* 49, 'red'))
		print(colored('|\t\tAuthor  : Lol_Sec07\t\t|', 'red'))
		print(colored('|\t\tProject : Encrypted File\t|', 'red'))
		print(colored('|\t\tContact : +601112268639\t\t|', 'red'))
		print(colored('='*49, 'red'))
		print(colored('[1] Change directory (shell)', 'green'))
		print(colored('[2] Encrypt / Decrypt single file', 'green'))
		print(colored('[3] Encrypt / Decrypt recursively', 'green'))
		print(colored('[0] Exit\n', 'green'))
		print(colored(' >> Current directory: ' + curDir + '\n'))
...
def cryptOne(act, password, file):
	...
	if act == 'encrypt': newfile = file + '.Lol'
	if act == 'decrypt': 
		try:
...
def cryptAll(act, password, folder):
	...
	for x in glob.glob(fd, recursive=True):
		...
		if act == 'encrypt': fullnewf = os.path.join(folder, x + '.Lol')
		if act == 'decrypt':
